...
if ( !function_exists( 'dw_minion_setup' ) ) {
	function dw_minion_setup() {
		...
		add_theme_support( 'post-thumbnails' );
		add_image_size( 'excerpt', 323, 323, true );
	}
}
...
function new_excerpt_more($more) {
       global $post;
	return ' <a class="article-more" href="'. get_permalink($post->ID) . '">read</a>';
}
...
function get_excerpt($count, $post){
	...
	$excerpt = $excerpt.'... <a class="article-more" href="'.$permalink.'">read</a>';
	return $excerpt;
}


if($_ENV["ENVIRONMENT"] == "local") {
  include "helper/debug.php";
}
